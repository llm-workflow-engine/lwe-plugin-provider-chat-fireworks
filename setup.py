...
with open(path.join(FILE_DIR, 'lwe_plugin_provider_chat_fireworks', 'version.py')) as f:
    version = re.match(r'^__version__ = "([\w\.]+)"$', f.read().strip())[1]

setup(
    name="lwe-plugin-provider-chat-fireworks",
    version=version,
    author="Chad Phillips",
    author_email="chad@apartmentlines.com",
    description="LWE plugin: Chat Fireworks Provider plugin",
    ...
    entry_points={
        "lwe_plugins": [
            "lwe_plugin_provider_chat_fireworks = lwe_plugin_provider_chat_fireworks.plugin:ProviderChatFireworks"
        ]
    },
)
